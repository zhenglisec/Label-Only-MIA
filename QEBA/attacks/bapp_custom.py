    ...
    * ability to specify the batch size
    ...
    def log_step(self, step, distance, message='', always=False, a=None, perturbed=None, update=None, aux_info=None):
        ...
        if a is not None:
           ...
           ## np.savez('./OEBA/BAPP_result/perturbed%s%d.npz' % (self.suffix, step), pert=perturbed.transpose(1,2,0),
           ##          info=np.array([a._total_prediction_calls, distance]))
            ...
            ##fig.savefig('./OEBA/BAPP_result/%sstep%d.png' % (self.suffix, step))
            plt.close(fig)
            if update is not None:
                ...
                plt.axis('off')
                plt.title('Call %d Distance %f'%(a._total_prediction_calls, distance))
                ##fig.savefig('./OEBA/BAPP_result/update%d.png'%step)
                plt.close(fig)
            #Saliency map
            #import cv2
            ...
            #
            ##self.printv("Call:", a._total_prediction_calls, "Saved to", './OEBA/BAPP_result/%sstep%d.png' % (self.suffix, step))
